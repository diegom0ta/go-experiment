...
func (r *OwnerRepository) GetOwnerByEmail(email string) (*domain.Owner, error) {
	...
	result := database.DB.Preload("Wallets").First(&owner, "email = ?", email)
	if result.Error != nil {
...
func (r *OwnerRepository) UpdateOwner(owner *domain.Owner) error {
	...
	err := database.DB.Save(owner).Error
	if err != nil {
		logger.WithFields(logrus.Fields{
			"owner_id": owner.ID,
			"error":    err.Error(),
	...
	logger.WithField("owner_id", owner.ID).Info("Owner updated successfully")
	return nil
}
